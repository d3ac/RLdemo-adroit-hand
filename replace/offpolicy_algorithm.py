...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _dump_logs(self, save_process=False) -> None:
        """
        Write log.
        """
        ...
        if save_process:
            self.reward_list_process.append(safe_mean([ep_info["r"] for ep_info in self.ep_info_buffer]))
            if self.reward_list_process[-1] > self.max_reward:
                self.max_reward = self.reward_list_process[-1]
                self.save("model_best")
            pd.DataFrame(self.reward_list_process).to_csv("reward_list_process.csv")
